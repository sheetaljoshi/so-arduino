...
#define MM_PER_REV              236    // 235.62  Set to the number of mm per wheel revolution (Hence : Diameter * Pi)

#define ENCODER_COUNTS_90_DEG   291.5   // Set to the number of encoder pulses to make a 90 degree turn
...
void loadCommandQueue() {
  ...
  // There is a maximum speed.  Testing will be required to learn this speed
      ...
      cmdQueue.add(VEHICLE_SET_TURN_SPEED,180);     // Speed used for left or right turns
      cmdQueue.add(VEHICLE_SET_ACCEL,300);         // smaller is softer   larger is quicker and less accurate moves
      ...
      cmdQueue.add(VEHICLE_TURN_LEFT, 90); 
      cmdQueue.add(VEHICLE_FORWARD,1500); 
      cmdQueue.add(VEHICLE_TURN_RIGHT, 90);
      ...
      cmdQueue.add(VEHICLE_TURN_LEFT, 90);
      cmdQueue.add(VEHICLE_FORWARD,1400); 
      cmdQueue.add(VEHICLE_TURN_LEFT, 90);
      cmdQueue.add(VEHICLE_FORWARD,1000); 
      cmdQueue.add(VEHICLE_TURN_LEFT, 90);
      cmdQueue.add(VEHICLE_FORWARD,1500); 
...
class MotionLogic
{
  ...
  public:
    ...
    void updateMotion(long usecElapsed) {
      ...
      //if (debugPrint) { Serial.print("pwmLoopI:"); Serial.print(pwmLoopI); Serial.print(" and ") ; Serial.print("pwmLoopP:"); Serial.print(pwmLoopP); }
...
void setup() {
  ...
  if (DISPLAY_PRESENT) {    Serial.println(F("Display init()"));
